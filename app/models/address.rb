class Address < ApplicationRecord
  ...
  with_options presence: true do
    validates :firstname, :lastname, :firstname_read, :lastname_read, :zip, :prefecture, :city, :address_line, presence: {message: "が空欄です"}
    validates :zip, format: { with: /\A\d{3}\-?\d{4}\z/,  message: "は半角数字７桁で入力してくだい"} 
    validates :firstname, :lastname, format: {with:/[一-龥ぁ-ん]/, message: "名前を入力してください"}
    validates :city, format: { with: /\A[一-龥ぁ-ん]/, message: "は全角で入力してください"}
    validates :firstname_read, :lastname_read, format: { with:/\A[ぁ-んー－]+\z/, message: "ひらがなで入力してください"}
    validates :prefecture, format: { with: /[一-龥]/,  message: "を入力して下さい"} , length: { maximum:3}
